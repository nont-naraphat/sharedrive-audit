...
from groups_service import (run_groups_audit, GROUPS_STATE, load_workspaces)
...
CFG = {
    ...
    # ---- group-mail audit ----
    "groups_cron": os.getenv("GROUPS_CRON", "30 2 * * *"),   # หลัง drive audit นิดนึง
    "groups_run_on_start": os.getenv("GROUPS_RUN_ON_START", "false").lower() == "true",
...
def _groups_job():
    print("[scheduler] เริ่ม crawl group-mail ตามเวลา")
    try:
        run_groups_audit(CFG["out"])
    except Exception as e:  # noqa
        print(f"[scheduler] groups error: {e}")
...
async def lifespan(app: FastAPI):
    try:
        ...
        scheduler.add_job(_groups_job,
                          CronTrigger.from_crontab(CFG["groups_cron"]),
                          id="nightly-groups", replace_existing=True)
        scheduler.start()
        print(f"[i] scheduler on — audit '{CFG['cron']}', "
              f"groups '{CFG['groups_cron']}' ({scheduler.timezone})")
    except Exception as e:  # noqa
        print(f"[!] scheduler ไม่ทำงาน: {e}")
    if CFG["run_on_start"] and not os.path.exists(os.path.join(CFG["out"], "audit.json")):
        import threading
        threading.Thread(target=_job, daemon=True).start()
    if CFG["groups_run_on_start"] and not os.path.exists(os.path.join(CFG["out"], "groups.json")):
        import threading
        threading.Thread(target=_groups_job, daemon=True).start()
    yield
...
# -------------------------------------------------- Group mail (multi-workspace)
@app.get("/api/workspaces")
def workspaces():
    """รายชื่อ workspace ที่ตั้งไว้ (ไว้ทำ dropdown สลับ)"""
    try:
        wss = load_workspaces()
    except Exception as e:  # noqa
        raise HTTPException(500, f"อ่าน WORKSPACES ไม่ได้: {e}")
    return {"workspaces": [
        {"id": w["id"], "name": w["name"], "admin": w["admin"],
         "domains": sorted(w["domains"])}
        for w in wss]}
...
def groups_status():
    ...
    job = scheduler.get_job("nightly-groups")
    next_sync = None
    if job and job.next_run_time:
        next_sync = job.next_run_time.isoformat(timespec="seconds")
    return {**GROUPS_STATE, "hasData": os.path.exists(data_path),
            "cron": CFG["groups_cron"], "next_sync": next_sync}


@app.get("/api/groups")
def get_groups(ws: str | None = None):
    """คืน groups.json ทั้งก้อน หรือกรองเฉพาะ workspace เดียวถ้าใส่ ?ws="""
    path = os.path.join(CFG["out"], "groups.json")
    if not os.path.exists(path):
        raise HTTPException(404, "ยังไม่มีข้อมูล — กด Sync group-mail ก่อน")
    if not ws:
        return FileResponse(path, media_type="application/json")
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    data["workspaces"] = [w for w in data.get("workspaces", []) if w["id"] == ws]
    return data
...
def groups_refresh(bg: BackgroundTasks):
    if GROUPS_STATE["running"]:
        raise HTTPException(409, "กำลัง sync group-mail อยู่แล้ว")
    bg.add_task(run_groups_audit, CFG["out"])
    return {"status": "started"}
...
def groups_export():
    ...
    if not os.path.exists(path):
        raise HTTPException(404, "ยังไม่มีไฟล์ export — กด Sync group-mail ก่อน")
    return FileResponse(
        path,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
